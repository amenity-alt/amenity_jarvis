...
屏幕左右各有一个 **全息面板**（悬浮窗口，可拖动、延伸到屏幕顶部）：
- 左侧 **环境信息**：时间、日期、天气（自动定位、30 分钟刷新）、电池、负载、内存、磁盘、网络、IP、运行时间、进程数、系统版本；带**仪表条**（电池/内存/磁盘使用率）和**趋势曲线**（负载、网络延迟）
- 右侧 **系统状态**：Jarvis 状态（待机/聆听/思考/回复）、模型名、接口延迟、会话轮数、进程，以及**接口延迟/电量趋势曲线**和滚动**活动日志**（最近的指令与回复）
- 右侧还有**实时对话输出**：对话逐条显示，Jarvis 回答时**逐字流式上屏**（DeepSeek 流式接口），像真正的 AI 控制台
- 每 5 秒刷新，历史数据保留 40 个采样点（约 3 分钟曲线）
